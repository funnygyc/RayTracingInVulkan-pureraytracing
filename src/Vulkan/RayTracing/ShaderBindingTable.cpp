...
namespace
{
		...
		const uint8_t* const shaderHandleStorage)
	{
		...
		for (const auto& entry : entries)
		{
			...
			pDst += entrySize;
			std::cout << "entry.GroupIndex : " << entry.GroupIndex << std::endl;
			std::cout << "entrySize : " << entrySize << std::endl;
			std::cout << "entry.InlineData.size() : " << entry.InlineData.size() << std::endl;
	...
	hitGroupSize_(hitGroups.size() * hitGroupEntrySize_)
{
	...
	std::cout << "shaderHandleStorage : " << shaderHandleStorage.size() << std::endl;
	for(int j=0;j<shaderHandleStorage.size();j++){
		std::cout << std::hex << shaderHandleStorage[j] << " " << std::dec;
	}
	std::cout << std::endl;
	...
	std::cout << "sbtSize : " << sbtSize << std::endl;
	// std::cout << "sbtSize : " << sbtSize << std::endl;
	// for(int j=0;j<(sbtSize/sizeof(int));j++){
	// 	std::cout << std::hex << int(reinterpret_cast<int *>(pData1)[j]) << " " << std::dec;
	// }
