...
{
	std::cout << "Raytracing::Application::SetPhysicalDevice " << std::endl;
	// Required extensions.
...
void Application::OnDeviceSet()
{
	...
	std::cout << "Raytracing::Application::OnDeviceSet" << std::endl;
	deviceProcedures_.reset(new DeviceProcedures(Device()));
	std::cout << "deviceProcedures finish" << std::endl;
	rayTracingProperties_.reset(new RayTracingProperties(Device()));
	std::cout << "RayTracingProperties finish" << std::endl;
}
...
void Application::CreateAccelerationStructures()
{
	...
	SingleTimeCommands::Submit(CommandPool(), [this](VkCommandBuffer commandBuffer)
	{
		...
		std::cout << "CreateBottomLevelStructures finish" << std::endl;
		CreateTopLevelStructures(commandBuffer);
		std::cout << "CreateTopLevelStructures finish" << std::endl;
	});
...
{
	std::cout << "Raytracing::Application::createSwapchain" << std::endl;
	Vulkan::Application::CreateSwapChain();
	std::cout << "Raytracing::Application::CreateOutputImage" << std::endl;
	CreateOutputImage();
	...
	std::cout << "Raytracing::Application::shaderBindingTable_ finish" << std::endl;
}
...
void Application::CreateBottomLevelStructures(VkCommandBuffer commandBuffer)
{
	...
	for (const auto& model : scene.Models())
	{
		...
		// model.Procedural() ? std::cout << "aabbOffset : " << aabbOffset << std::endl : std::cout << "vertexOffset : " << vertexOffset << std::endl;
...
void Application::CreateTopLevelStructures(VkCommandBuffer commandBuffer)
{
	...
	for (const auto& model : scene.Models())
	{
		...
		instanceId++;
		std::cout << "instances create" << std::endl;
	}
	...
	std::cout << "topAS finish" << std::endl;
	// Allocate the structure memory.
	const auto total = GetTotalRequirements(topAs_);
	std::cout << "GetTotalRequirements finish" << std::endl;
	...
	std::cout << "topbuffer finish" << std::endl;
	...
	std::cout << "topScratchBuffer finish" << std::endl;
	...
	std::cout << "before Generate the structures" << std::endl;
	// Generate the structures.
	topAs_[0].Generate(commandBuffer, *topScratchBuffer_, 0, *topBuffer_, 0);
	std::cout << "Generate the structures finish : " << topAs_[0].Handle() << std::endl;
